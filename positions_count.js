...
Promise.all(promises)
 .then(function(data){
	...
	var total = 0
	 for(var d in data){	
		// console.log(data[d])
		 layerGeos[mtfccsFile[d]]=data[d]
		 total+=mtfccsPositionsCount[mtfccsFile[d]]
		 d3.select("#info").append("div")
		 .html(mtfccsFileNames[mtfccsFile[d]]+" "+ mtfccsPositionsCount[mtfccsFile[d]]+" entries")
		 .attr("class","layerKey")
		 .attr("id",mtfccsFile[d])
		 .style('background-color',layerColors[mtfccsFile[d]])
		 .on("click",function(){
			 var clickedLayer = d3.select(this).attr("id")
			 
			 if(layerVisiblities[clickedLayer]=="none"){
				 map.setLayoutProperty(clickedLayer, 'visibility', 'visible');
				 d3.select(this).style("opacity",1)
				 layerVisiblities[clickedLayer]="visible"
				 
			 }else{
				 map.setLayoutProperty(clickedLayer, 'visibility', 'none');
				 d3.select(this).style("opacity",.5)
				 layerVisiblities[clickedLayer]="none"
			 }
...
//var maxBounds = [[-74.3, 40.5],[-73.5, 41]]
    ...
    map = new mapboxgl.Map({
		container: 'map',
		style:"mapbox://styles/jiaz-usafacts/cl6b3lj0o000r15nj6rabo5cw",
		zoom: 4,
